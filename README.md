...
> 楊程軒：這次從傳統的Tool Calling轉向MCP架構，我最深刻的體會是開發邏輯的解放。過去在實作工具調用時，開發者往往像是一個翻譯官，必須戰戰兢兢地將Python 函式手動轉換成大語言模型看得懂的 JSON Schema。一旦工具數量增加或參數變動，維護成本就會呈指數增長。但在引入 MCP 後，這種繁瑣的過程消失了。透過 @mcp.tool() 裝飾器，我們能直接將開發重心回歸到工具本身的邏輯實現，而不需要去煩惱底層通訊與格式對齊的細節。
...
> 楊程軒：當多個工具同時存在時，Gemini Agent 就像是一個擁有多種技能的實習生，如果我的指令不夠具體，模型很容易在調用時機上產生混淆。我花了不少時間反覆測試與微調 System Prompt，學習如何撰寫更具判斷邏輯的註解。這讓我意識到，在 AI Agent 的開發中，自然語言的規格定義與程式邏輯同樣重要，這也是從工程師轉 AI應用開發者的思維轉換過程。
...
> 楊程軒：傳統 Tool Calling 與 MCP 的主要差異在於架構的解耦與開發流程的簡化。在上週的實作中，工具邏輯與模型呼叫必須高度整合在同一個專案，且開發者需手動編寫符合模型規範的 JSON Schema，導致維護困難且難以跨模型共用。引入 MCP 後，工具被封裝為獨立運行的伺服器，透過標準協定與 Agent 通訊，實現了邏輯與決策的完全分離。這種架構支援動態探索機制，Agent 能在連線時自動獲取工具定義，開發者僅需專注於 Python 函數的邏輯實現與註解，無需再手動處理資料格式轉換，大幅提升了工具開發的效率與系統整體的擴展性。